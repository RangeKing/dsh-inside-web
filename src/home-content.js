...
export const NOTES=[
 ...
 {id:'shell',path:'packages/shell/tool-bash/README.md',lines:'55',text:['Shell 返回命令输出与退出信息；非零退出码交给 Agent 判断，不能直接当作任务完成。','Shell returns command output and exit information. The agent interprets nonzero exits; they do not establish task completion.']},
 ...
 {id:'compaction',path:'packages/compaction/compaction-basic/README.md',lines:'90-113',text:['启用自动压缩与裁剪插件后，每个 Step 前检查压力；裁剪足够时可跳过摘要。','With auto-compaction and pruning configured, pressure is checked before each step; successful pruning may avoid a summary.']},
