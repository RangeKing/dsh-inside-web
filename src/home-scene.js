...
export class HomeScene {
 ...
 sample(chapter,lessonPositions,provider,mobile){
   if(chapter===1||chapter===2)return this.teaching.sample(chapter,lessonPositions[chapter],provider,mobile);
 ...
 update(chapter,pose,lessonPositions,provider,mobile){
   const next=Math.min(6,chapter+1),a=this.sample(chapter,lessonPositions,provider,mobile),b=pose.blend>0?this.sample(next,lessonPositions,provider,mobile):a;
