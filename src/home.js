...
export class HomeExperience {
 ...
 render(){
   ...
   ${chapters.map((c,i)=>`<section id="chapter-${c.slug}" class="ocean-chapter chapter-${i}" data-chapter-index="${i}"><div class="chapter-sticky"><div class="chapter-copy">
     ...
     <div class="chapter-detail">${i===0?this.requestMarkup():''}${c.thread?`<p class="request-thread"><span>${T(['你的请求','Your request'])}</span>${T(c.thread)}</p>`:''}${i===1||i===2?this.lessonMarkup(i):''}<div class="model-legend"></div><p class="chapter-body">${T(c.body)}</p>${i===6?this.resultMarkup():''}
