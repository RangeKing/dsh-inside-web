...
const REQUEST=['帮我看看采购单还缺什么，顺便把缺的补进去。','Check what’s missing from my shopping list, and add it.'];
const chapters=[
...
export class HomeExperience {
 constructor(){
   ...
   this.root.addEventListener('input',e=>{const scrub=e.target.closest('[data-lesson-scrub]');if(scrub)this.lessonAction('seek',Number(scrub.value));},{signal});
 ...
 render(){
   ...
   ${chapters.map((c,i)=>`<section id="chapter-${c.slug}" class="ocean-chapter chapter-${c.look??i}" data-chapter-index="${i}"><div class="chapter-sticky"><div class="chapter-copy">
     <p class="ocean-eyebrow">${i===0?'DeepSeek Harness':T([['','模块化设计','执行循环','工具与审批','插件装卸','上下文管理','分工与扩展','开始探索'][i],['','Modular by design','The agent loop','Tools & boundaries','Plugin lifecycle','Context, considered','Better together','Your next step'][i]])}</p>
     <${i===0?'h1':'h2'}>${T(c.title).split('\n').map((line,j)=>`<span class="headline-line line-${j}">${line}</span>`).join(window.DSHLocale?.language==='en'?' ':'')}</${i===0?'h1':'h2'}>${c.thread?`<p class="request-thread"><span>${T(['你的请求','Your request'])}</span>${T(c.thread)}</p>`:''}
     <div class="chapter-detail">${i===0?this.requestMarkup():''}${lessonFor(i)?this.lessonMarkup(i):''}<div class="model-legend"></div><p class="chapter-body">${T(c.body)}</p>${i===LAST?this.resultMarkup():''}
     ...
     ${i===LAST?`<button class="dive-button final-cta" data-mode="library">${T(['进入八个案例','Explore all eight cases'])}<span>↗</span></button>`:''}
 ...
 requestMarkup(){const T=x=>this.text(x);return `<figure class="request-bubble"><figcaption>${T(['跟着这一句话往下看','Follow this one request'])}</figcaption><p><span class="request-files" aria-label="${T(['附带文件','Attached files'])}">${T(['采购单.txt','shopping-list.txt'])} · ${T(['周末菜单.txt','weekend-menu.txt'])}</span>${T(REQUEST)}</p></figure>`;}
 resultMarkup(){const T=x=>this.text(x);return `<figure class="request-result"><figcaption>${T(['这句话最后的结果','Where the request ended up'])}</figcaption><p class="request-echo">${T(REQUEST)}</p><p class="request-reply">${T(['菜单要用番茄和鸡蛋，采购单上没有，已经补上了。','The menu needs tomatoes and eggs. They were missing from the list, so I added them.'])}</p><ol class="request-list">${T([['牛奶：2 盒','大米：1 袋','<ins>番茄：4 个</ins>','<ins>鸡蛋：6 个</ins>'],['Milk: 2 cartons','Rice: 1 bag','<ins>Tomatoes: 4</ins>','<ins>Eggs: 6</ins>']]).map(x=>`<li>${x}</li>`).join('')}</ol><small>${T(['教学示例：文件内容和回复都是为本页编写的，不是模型实际输出。','Teaching example: the files and reply were written for this page, not produced by a model.'])}</small></figure>`;}
