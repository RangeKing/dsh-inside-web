...
const required=[...new Set(Array.from({length:8},(_,i)=>world.chapterAssets(i)).flat().concat(['orca_hull','support_node']))];
await Promise.all(required.map(id=>world.load(id)));
function snapshot(p){world.setState({chapter:Math.min(7,Math.floor(p)),progress:p%1});world.motion.set(p,true);Object.values(world.players).forEach(player=>player.playing=false);world.draw(performance.now());world.scene.updateMatrixWorld(true);
